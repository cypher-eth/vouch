...
abstract contract VouchDescriptor is NFTDescriptor {
     ...
     */
    function dataURI(uint256 tokenId, uint256 _lastUpdate, address _sender, string memory _msg) public view returns (string memory) {
        string memory name = string(abi.encodePacked('Vouch #', toString(tokenId)));

        string memory image = string(abi.encodePacked('data:image/svg+xml;base64,', Base64.encode(bytes(_vouchImage(tokenId, _lastUpdate,_sender,_msg )))));
     ...
     */
    function _vouchImage(uint _tokenId, uint256 _lastUpdate, address _sender, string memory _msg) internal view returns (string memory output) {
        output = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: #004440; font-family: serif; font-size: 15px; }</style><rect width="100%" height="100%" fill="#eee5d3" /><text x="250" y="40" class="base">';
        ...
        output = string(abi.encodePacked(output, toHexString(_sender), '</text><text x="240" y="330" class="base">vouch # ' ,toString(_tokenId) , '</text></svg>'));
    }
